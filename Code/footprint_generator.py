...
class Footprint:
    ...
    def plot_zones(self):
        ...
        ax.pcolormesh(
            self.x_mesh, self.y_mesh, np.where(self.observed_masks[0], self.raster_map, np.nan), cmap='Blues', shading='auto'
        )
        ax.pcolormesh(
            self.x_mesh, self.y_mesh, np.where(self.observed_masks[1], self.raster_map, np.nan), cmap='Purples', shading='auto'
        )
        ax.pcolormesh(
            self.x_mesh, self.y_mesh, np.where(self.superpos_masks[0], self.raster_map, np.nan), cmap='Reds', shading='auto'
        )
